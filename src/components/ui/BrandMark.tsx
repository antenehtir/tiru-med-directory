...
export function BrandMark({ tone = "default" }: { tone?: "default" | "inverse" }) {
  ...
  return (
    <Link aria-label="Tiru Medical Directory home" className="flex min-w-0 items-center gap-2 py-1" href="/">
      <TiruMark className="h-11 w-auto shrink-0 sm:h-12" inverse={inverse} />
      <span className="flex min-w-0 flex-col leading-none">
        <span className={`font-body text-[1.45rem] font-extrabold tracking-[-0.035em] sm:text-[1.6rem] ${ink}`}>Tiru</span>
