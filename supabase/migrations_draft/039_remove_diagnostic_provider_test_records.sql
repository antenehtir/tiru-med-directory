...
--
-- Removes six test records from public.diagnostic_providers.
--
-- All six are named "Test Diagnostic <Greek letter> <type>" — Alpha Laboratory,
-- Eta Imaging Center, Zeta Radiology Center, Omega Pathology Service, Kappa
-- Mixed Center, Lambda Home Sample Collection. They are the entire contents of
-- the table: a count of rows whose display_name does NOT start with "Test "
-- returns 0, so this is not a case of test rows mixed in with real ones.
--
...
-- read path picking the table back up and publishing six fake laboratories.
--
...
-- Every row, so the six below can be confirmed as the whole table.
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Guarded on the name prefix rather than the six ids, so a row added since
-- this file was written cannot be caught by it.
BEGIN;
